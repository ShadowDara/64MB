...
fn main() -> io::Result<()> {
    ...
    let bottom_reserved = 2; // z. B. Status + Eingabe unten
    ...
    loop {
        // TODO
        // Komplettees Fenster neu schreiben bei Terminal resize

        // Eingabezeile aktualisieren (nur wenn nötig)
        stdout.execute(cursor::MoveTo(0, input_y))?;
        stdout.execute(Clear(ClearType::CurrentLine))?;
        print!(":{}", input_buffer);
        stdout.flush()?;

        // ⬇Warten, bis wirklich ein Event kommt (blockierend!)
            ...
            match key.code {
                ...
                KeyCode::Enter => {
                    ...
                    if input_buffer == "quit" {
                        ...
                        // Ausgabe oberhalb der Eingabezeile
                        stdout.execute(cursor::MoveTo(0, input_y - 1))?;
                        stdout.execute(Clear(ClearType::CurrentLine))?;
                        println!("Command: {}", input_buffer);
                        input_buffer.clear();
                    }
                }
                KeyCode::Char(c) => {
